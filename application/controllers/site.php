...
class Site extends CI_Controller {
    ...
    public function galerias() {
        ...
        $this->mostrar_por_pagina = 16;		
        $this->listGalerias        = $this->galerias->get_galerias($this->mostrar_por_pagina, $this->start)->result();
        $this->numero_itens        = $this->db->get('galerias')->num_rows();
        $config['base_url']        = base_url().'site/galerias';
        $config['total_rows']      = $this->numero_itens;
        $config['per_page']        = $this->mostrar_por_pagina;
        $config['num_link']        = 4;		
        $config['uri_segment']     = 3;	
    ...
    public function agenda() {
        ...
        $this->mostrar_por_pagina = 24;		
        $this->listShows    = $this->agenda->get_shows($this->mostrar_por_pagina, $this->start)->result();
        $this->numero_itens = $this->db->get('agenda')->num_rows();
        $config['base_url']        = base_url().'site/agenda';
        $config['total_rows']      = $this->numero_itens;
        $config['per_page']        = $this->mostrar_por_pagina;
        $config['num_link']        = 3;		
        $config['uri_segment']     = 3;	
        ...
        set_tema('titulo', '.:: Agenda ::.');
        set_tema('description', get_setting('descricao_site'));
        set_tema('keywords', get_setting('keywords'));
        set_tema('conteudo', load_modulo( 'agenda', 'listar', 'site'));
        load_template();
    }
    public function detalhes_agenda() {
        ...
        set_tema('titulo', '.:: Detalhes da agenda ::.');
        set_tema('description', get_setting('descricao_site'));
        set_tema('keywords', get_setting('keywords'));
        set_tema('conteudo', load_modulo( 'agenda', 'detalhes', 'site', $data));
        load_template();
    }
    public function faleconosco(){
            ...
            set_tema('keywords', get_setting('keywords'));
            set_tema('conteudo', load_modulo( 'faleconosco', 'faleconosco', 'site'));
            load_template();
    }
